...
const Ship = function(options = { radius: 13 }) {
  ...
  this.speedUp2 = false;
  this.bulletsInPlay = [];
  this.speedMultiplier = 3; // Increase from 1.5 to 3
...
Ship.prototype.fireBullet = function() {
  ...
  let bulletVel;
  if (this.name === 'defender') {
    bulletVel = [0, -6]; // Change back to -6 from -12
    bulletColor = "#FF00FF";
  } else {
    bulletVel = [0, 6]; // Change back to 6 from 12
    bulletPosY += 30;
...
Ship.prototype.reverse = function() {
  let newVel = Math.abs(this.vel[0]) + 0.03; // Adjust from 0.04 to 0.03
  if (this.vel[0] > 0) {
    newVel = 0 - newVel;
    this.vel = [newVel, 0];
    this.pos[0] -= 7.5; // Adjust from 10 to 7.5
  } else {
    this.vel = [newVel, 0];
    this.pos[0] += 7.5; // Adjust from 10 to 7.5
  }
  this.pos[1] += 30; // Adjust from 40 to 30
};

Ship.prototype.increaseSpeed = function() {
  let newVel = Math.abs(this.vel[0]) + 0.0015; // Adjust from 0.002 to 0.0015
  if (this.vel[0] < 0) {
    newVel = 0 - newVel;
    this.vel = [newVel, 0];
  } else {
...
Ship.prototype.move = function() {
  ...
  if (this.pos[0] > this.canvasSize[0] - 20) {
    ...
    this.pos[0] += this.vel[0] * (this.speedMultiplier * 0.75); // Adjust to 75% of current speed
    this.pos[1] += this.vel[1] * (this.speedMultiplier * 0.75); // Adjust to 75% of current speed
  }
