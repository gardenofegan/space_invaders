...
GameView.prototype.handleGamepadInput = function() {
  ...
  requestAnimationFrame(this.handleGamepadInput.bind(this));
};
  ...
  const gamepadHandler = () => {
    ...
    if (gamepad) {
      ...
      if (newLeftPressed !== this.gamepadState.leftPressed) {
        ...
        if (newLeftPressed) {
          this.handleKeyDown({ keyCode: 37 }); // Left arrow key code
        } else {
          this.handleKeyUp({ keyCode: 37 });
        }
      }
      ...
      if (newRightPressed !== this.gamepadState.rightPressed) {
        ...
        if (newRightPressed) {
          this.handleKeyDown({ keyCode: 39 }); // Right arrow key code
        } else {
          this.handleKeyUp({ keyCode: 39 });
        }
      }
      ...
      if (newFirePressed !== this.gamepadState.firePressed) {
        ...
        if (newFirePressed) {
          this.handleKeyDown({ keyCode: 32 }); // Spacebar key code
        } else {
          this.handleKeyUp({ keyCode: 32 });
        }
      }

      // Handle start button
      if (newStartPressed !== this.gamepadState.startPressed) {
        this.gamepadState.startPressed = newStartPressed;
        if (newStartPressed) {
